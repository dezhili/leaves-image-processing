...
def get_leave_serration(contours):
    ...
    total_curvature_mean = get_leave_curvature.curvature_splines(contours[:, 0], contours[:, 1], error=0.1)
    total_curvature_mean = np.mean(total_curvature_mean)

    return ser4_idx, ser4_deepest_idx, serration_numbers, serration_depths, serration_widthes, curvatures, total_curvature_mean
...
def save_to_csv(serration_numbers, serration_depths, serration_widthes, curvatures, total_curvature_mean, name_str):
    # 将深度宽度等结果保存到文件
    results_list = list()
    # 写入参数配置
    results_list.append(['serration_idx', 'serration_depth', 'serration_width', 'curvature_mean'])
    ...
    for i in range(serration_numbers+1):
        ...
        if serration_idx < serration_numbers:
            ...
            curvature_mean = curvatures[serration_idx]
            serration_idx += 1
            results_list.append([serration_idx, serration_depth, serration_width, curvature_mean])
